...
(function () {
	...
	tinymce.create('tinymce.plugins.AutoCompletePlugin', {
		...
		init: function (ed, url) {
			 ...
			 */
			function keyUpEvent(ed, event) {
				if ((!autocomplete_data.visible && event.keyCode != ESC_KEY && event.keyCode != ENTER_KEY) || (event.keyCode != DOWN_ARROW_KEY && event.keyCode != UP_ARROW_KEY && event.keyCode != ENTER_KEY && event.keyCode != ESC_KEY)) {
			 ...
			 */
			function keyPressEvent(ed, event) {
				if (event.keyCode == ENTER_KEY && autocomplete_data.cancelEnter) {
			 ...
			 */
			function keyDownEvent(ed, event) {
				if (autocomplete_data.visible) {
					...
					// onMatch callback
					if (autocomplete_data.onMatch && END_WORD_KEYS.indexOf(event.keyCode)) {
			...
			ed.onKeyUp.addToTop(keyUpEvent(ed, event));
			ed.onKeyDown.addToTop(keyDownEvent(ed, event));
			ed.onKeyPress.addToTop(keyPressEvent(ed, event));
			ed.onClick.add(clickEvent(ed, event));	
//            ed.on('KeyUp', addToTop(keyUpEvent));
//			ed.on('KeyDown', addToTop(keyDownEvent));
//			ed.on('KeyPress', addToTop(keyPressEvent));
//			ed.on('Click', add(clickEvent));
		},
