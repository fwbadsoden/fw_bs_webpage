...
$(function() {
	$('#<?=$userUsername['id']?>').bind('change', function(e) {
			$.getJSON('<?=base_url('user/user_admin/json_userattr_unique')?>/backend/username/0/' + $('#<?=$userUsername['id']?>').val(),
				function(data) {
					if(data == 0)
	...
	$('#<?=$userEmail['id']?>').bind('change', function(e) {
			$.getJSON('<?=base_url('user/user_admin/json_userattr_unique')?>/backend/email/0/' + $('#<?=$userEmail['id']?>').val(),
				function(data) {
					if(data == 0)
